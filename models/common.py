...
# LSKA   
class Conv_LSKA(nn.Module):
    # Standard convolution
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super(Conv_LSKA, self).__init__()
        self.conv0h = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        self.att = LSKA(c2)

    def forward(self, x):
        return self.att(self.act(self.bn(self.conv0h(x))))

    def fuseforward(self, x):
        return self.att(self.act(self.conv0h(x)))
